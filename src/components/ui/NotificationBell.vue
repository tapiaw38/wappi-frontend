...
const loadNotifications = () => {
  const saved = localStorage.getItem('wappi_notifications')
  if (saved) {
    try {
...
const saveNotifications = () => {
  localStorage.setItem('wappi_notifications', JSON.stringify(notifications.value))
}
  ...
  <div class="notification-bell-container">
    ...
    >
      <div class="notifications-header">
        <h3>
          Notificaciones
          <span v-if="unreadCount > 0" class="notification-count-badge">{{ unreadCount }}</span>
        </h3>
        <Button
          v-if="unreadCount > 0"
